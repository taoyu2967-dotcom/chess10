# chess10d — 10×10 国际象棋变体：训练与对弈套件

[English](README.md) | 简体中文

> AlphaZero 风格自研训练管线 + GPU/NPU/CPU 三后端推理 + 网页与桌面对弈界面
...
**chess10d** 是在 Fairy-Stockfish [`variants.ini`](fsf/variants.ini) 中定义的 10×10 国际象棋变体（含自定义棋子 `d` 与扩展规则，走法详见该文件）。本仓库包含围绕它构建的完整套件：
...
> 架构演进：v2 引入 MANO 窗口注意力/GRN/rpb 主干；v3 修复 policy 编码非单射缺陷（POLICY_CH 100→160：基础走法 0–99、马连跳 100–131、升变 132–146、兜底 147），并把架构开关（ARCH_FLAGS）写进权重尾部，保证 JS / PyTorch / GPU 三实现不漂移。三实现对拍：JS↔PyTorch max|Δ|=1.3e-6，GPU↔CPU 7.6e-6。详见 [`docs/contracts/v3_contract.md`](docs/contracts/v3_contract.md) 与 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md)。
...
```bash
npm test            # 全量门禁（约 5-8 分钟）：编码单射 / 走法与攻击等价 / cnn+GPU 对拍 /
                    # e2e 冒烟（真实起服+WS）/ 集成门禁 / OpenVINO×NPU / 权重探针×4
npm run test:quick  # 纯 JS 核心门禁（约 1 分钟，无卡/无 python 环境可跑）
```

任何涉及网络结构、kernel 或 patch 层的改动，必须保持 `npm test` 全绿。
对拍矩阵与阈值见 [`docs/ARCHITECTURE.md`](docs/ARCHITECTURE.md) §2。

## 仓库结构

```
chess10.html            # 网页对弈前端（单文件，含 Magic V2 UI）
server/                 # 引擎、MCTS、后端、蒸馏与自对弈
  cl/kernels.cl         #   OpenCL kernel 源（纯 OpenCL C，可独立阅读编辑）
  paths.js              #   路径中枢——全仓唯一允许解析仓库根的位置
  tools/                #   基准与诊断工具（验收门禁已集中到 tests/）
tests/                  # 门禁套件：run_all.js 统一编排全部验收门禁
ov_train/               # PyTorch 训练器与架构定义 (az_model.py)
training/               # 本地守护进程、数据治理、损失面可视化
cloud/                  # 云端时代运维脚本与产物取回（历史档案，见 RUNBOOK）
...
docs/                   # ARCHITECTURE / RUNBOOK / CONFIG + 架构契约（v2/v3）
```
